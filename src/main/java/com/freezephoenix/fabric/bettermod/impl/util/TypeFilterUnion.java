 ...
 */
public final class TypeFilterUnion<B, T extends B> implements EntityTypeTest<B, T> {
	private final EntityTypeTest<B, ? extends T>[] filters;
	private final Class<B> klass;
	private final int size;

	@Contract(pure = true)
	private TypeFilterUnion(Class<B> klass, EntityTypeTest<B, ? extends T> @NotNull [] filters) {
	...
	@Override
	public @Nullable T tryCast(final B ent) {
		for (int i = 0; i < size; i++) {
			T cur = filters[i].tryCast(ent);
			if (cur != null) {
				return cur;
			}
		}
	...
	public static final class Builder<B, T extends B> {
		private final ReferenceArrayList<EntityTypeTest<B, ? extends T>> filters = new ReferenceArrayList<>();
		private final Class<B> klass;
		...
		public <C extends T> Builder<B, T> add(final EntityTypeTest<B, C> filter) {
			filters.add(filter);
			return this;
		}
